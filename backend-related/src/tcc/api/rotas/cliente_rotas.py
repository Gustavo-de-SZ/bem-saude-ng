    ...
    for cliente in clientes:
        resultado.append(
            ClienteResponse(
                ...
                ativo=cliente.usuario.ativo if cliente.usuario else True,
                empresa=cliente.empresa,
                avaliacao=cliente.avaliacao,
                servicos_ativos=cliente.servicos_ativos,
                servicos_concluidos=cliente.servicos_concluidos,
...
def obter_estatisticas_clientes(
    ...
    if user_profile == "PROFISSIONAL":
        # For professionals, return stats for their associated clients only
        cliente_tecnico_repo = ClienteTecnicoRepository(session)
        profissional_id = current_user.profissional.id
        clientes = cliente_tecnico_repo.listar_clientes_por_profissional(profissional_id)

        total = len(clientes)
        # Count active clients (those with associated usuario that is active)
        ativos = sum(1 for c in clientes if c.usuario and c.usuario.ativo)
        ...
        # For clients, return general stats (or could be customized)
        total = session.query(ModeloCliente).count()

        # Conta quantos clientes possuem usuario ativo = True (caso utilize a relacao)
        ativos = session.query(ModeloCliente).join(ModeloUsuario).filter(ModeloUsuario.ativo == True).count()
...
def criar_cliente_manual_tecnico(
    ...
    novo_cliente = ModeloCliente(
        ...
        avaliacao=int(dados.avaliacao) if dados.avaliacao is not None else 0,
...
def obter_cliente_por_email(
    ...
    # Buscar cliente por email, id ou nome
    if "@" in identificador:
        ...
        cliente = session.query(ModeloCliente).filter(ModeloCliente.nome_completo == identificador).first()
