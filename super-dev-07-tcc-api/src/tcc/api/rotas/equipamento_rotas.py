...
def criar_equipamento(
    ...
    # Validar se o cliente pertence ao profissional
    cliente_tecnico_repo = ClienteTecnicoRepository(session)
    if not cliente_tecnico_repo.eh_vinculado(cliente.id, current_user.profissional.id):
        raise HTTPException(status_code=HTTPStatus.FORBIDDEN, detail="Cliente não pertence ao profissional logado")
