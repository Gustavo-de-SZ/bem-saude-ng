...
def atualizar_servico(
    ...
    servico_antigo = repositorio.buscar_por_titulo_e_usuario(titulo, current_user.id)
    if not servico_antigo:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Serviço não encontrado"
        )
        
    foi_concluido = servico_antigo.status.value != 'Concluído' and dados.status == 'Concluído'
    ...
    if foi_concluido:
        try:
            transacao = ModeloTransacao(
                titulo=f"Pagamento - {servico_atualizado.titulo}",
                cliente=servico_atualizado.cliente,
                data=datetime.now().strftime("%d/%m/%Y"),
                valor=servico_atualizado.valor,
                status=StatusTransacaoEnum.RECEBIDO,
                usuario_id=current_user.id
            )
            ...
            session.rollback()
            # We don't rollback the service update here based on standard behavior, 
            # but if it needs to be fully atomic, we would.
