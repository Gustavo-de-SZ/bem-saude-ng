...
from src.tcc.api.rotas import (
    ...
    tecnicos_rotas, equipamento_rotas, notificacao_rotas
)
...
def criar_aplicacao() -> FastAPI:
    ...
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["http://localhost:4200", "http://localhost:3000", "http://localhost:8000"], allow_origin_regex=".*", # Allow all for dev
            ...
            with engine.begin() as conn:
                try:
                    ...
                    if 'usuario_id' in columns:
                        ...
                        conn.execute(text("ALTER TABLE clientes ALTER COLUMN usuario_id DROP NOT NULL"))
        ...
        if inspector.has_table('equipamentos'):
            with engine.begin() as conn:
                try:
                    ...
                    conn.execute(text("ALTER TABLE equipamentos ALTER COLUMN tipo TYPE VARCHAR(100)"))
                    conn.execute(text("ALTER TABLE equipamentos ALTER COLUMN tipo SET NOT NULL"))
                except Exception as e:
            ...
            with engine.begin() as conn:
                try:
                    ...
                    if 'data_registro' in columns:
                        ...
                        conn.execute(text("ALTER TABLE equipamentos ALTER COLUMN data_registro TYPE VARCHAR(20)"))
                        conn.execute(text("ALTER TABLE equipamentos ALTER COLUMN data_registro DROP NOT NULL"))
                except Exception as e:
                    logger.warning(f"Could not modify data_registro in equipamentos table to allow NULL: {e}")

        # NEW: Ensure anexo column exists in chamados table
        if inspector.has_table('chamados'):
            with engine.begin() as conn:
                try:
                    # Check if anexo column exists, if not add it
                    columns = [col['name'] for col in inspector.get_columns('chamados')]
                    if 'anexo' not in columns:
                        # Add the anexo column as TEXT NULL
                        conn.execute(text("ALTER TABLE chamados ADD COLUMN anexo TEXT NULL"))
                except Exception as e:
                    logger.warning(f"Could not add anexo column to chamados table: {e}")
