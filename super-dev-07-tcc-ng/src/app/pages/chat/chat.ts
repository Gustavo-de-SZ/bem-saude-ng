...
export class ChatComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    const updateDestinatario = () => {
        if (this.ticketDetalhes) {
            if (this.role === 'tecnico' && this.ticketDetalhes.cliente_nome) {
                this.destinatario = this.ticketDetalhes.cliente_nome;
                this.destinatarioIniciais = this.ticketDetalhes.cliente_nome.substring(0, 2).toUpperCase();
            } else if (this.role === 'cliente' && this.ticketDetalhes.profissional_nome) {
                this.destinatario = this.ticketDetalhes.profissional_nome;
                this.destinatarioIniciais = this.ticketDetalhes.profissional_nome.substring(0, 2).toUpperCase();
            }
        }
    };
    ...
    this.authService.user$.pipe(first()).subscribe(user => {
        if (user) {
            ...
            this.role = roles.length > 0 ? roles[0].toLowerCase() : 'tecnico';
            if (this.role === 'profissional') this.role = 'tecnico';
    ...
    interval(3000).pipe(
      startWith(0),
      switchMap(() => this.ticketId === 'GLOBAL-CHAT' ? of([]) : this.chatService.getMensagens(this.ticketId).pipe(
        catchError(err => {
  ...
  enviar() {
    ...
    this.chatService.enviarMensagemRole(this.ticketId, texto, this.meuUsuarioId, this.meuNome).subscribe({
      next: (novaMsg) => {
